...
class _BorrowedScreenState extends State<BorrowedScreen> {
  ...
  void initState() {
    ...
    cubit.initDB();
    cubit.getAllBorrowed(nameTable: tableBorrowed); // Fetch data
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<MainCubit, MainState>(
      listener: (context, state) {
        if (state is GetDataBorrowedFiledState) {
      ...
      builder: (context, state) {
        if (state is GetDataBorrowedLodedState) {
          return const Center(
            child: CircularProgressIndicator(),
          );
        }
        if (state is GetDataBorrowedSucessState) {
          return Scaffold(
            floatingActionButton: FloatingActionButton(
              onPressed: () {
                showFullScreenBottomSheetBorrowed(
                    context: context,
                    dateController:
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            ...
                            child: SecondaryCourseCard(
                              ...
                              iconsSrc:
                                  index % 2 == 0 ? imageCodeAss : imageAss,
                              id: transaction['id'],
