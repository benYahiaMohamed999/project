...
class _SpentScreenState extends State<SpentScreen> {
  ...
  void initState() {
    ...
    cubit.initDB();
    cubit.getAllSpent(nameTable: tableSpent); // Fetch data
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<MainCubit, MainState>(listener: (context, state) {
      if (state is GetDataSpentFiledState) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(state.error)),
        );
      }
    }, builder: (context, state) {
      if (state is GetDataSpentLodedState) {
        return const Center(
          child: CircularProgressIndicator(),
        );
      }
      if (state is GetDataSpentSucessState) {
        return Scaffold(
          floatingActionButton: FloatingActionButton(
            onPressed: () {
              showFullScreenBottomSheetSpent(
                context: context,
