...
class _PaidScreenState extends State<PaidScreen> {
  ...
  void initState() {
    ...
    cubit.initDB();
    cubit.getAllPaid(nameTable: tablePaid); // Fetch data
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<MainCubit, MainState>(listener: (context, state) {
      if (state is GetDataPaidFiledState) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(state.error)),
        );
      }
    }, builder: (context, state) {
      if (state is GetDataPaidLodedState) {
        return const Center(
          child: CircularProgressIndicator(),
        );
      }
      if (state is GetDataPaidSucessState) {
        return Scaffold(
          floatingActionButton: FloatingActionButton(
            onPressed: () {
              showFullScreenBottomSheetPaid(
                  context: context,
                  dateController:
